...
curl --version
if [[ $? == 127  ]]; then  yum -y install curl; fi
...
if [[ $install == true ]]; then
  bash <(curl -s https://raw.githubusercontent.com/HamzaMouh/install-fivem/main/install.sh) $runtime_link
else
  bash <(curl -s https://raw.githubusercontent.com/HamzaMouh/install-fivem/main/update.sh) $runtime_link
fi
